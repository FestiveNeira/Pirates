...
public class jump : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (this.gameObject.transform.position.y > (shadow.transform.position.y + 0.41))
        {
            UpdateGravityScale();
        }
        // catch
        if (this.gameObject.transform.position.y < (shadow.transform.position.y + 0.40))
        {
            rb.gravityScale = 0;
            rb.velocity = Vector2.zero;
            this.gameObject.transform.position = new Vector3(shadow.transform.position.x, shadow.transform.position.y + 0.4f, this.gameObject.transform.position.z);
            onGround = true;
        }
        // jump
    ...
    void FixedUpdate()
    {
        ...
        {
            UpdateGravityScale();
            this.gameObject.transform.position = new Vector3(this.gameObject.transform.position.x, this.gameObject.transform.position.y - (jumppos - shadow.transform.position.y), this.gameObject.transform.position.z);
            jumppos = shadow.transform.position.y;
        }
        else
        {
